...
Estás trabajando en el **Kit del Curso 2.4 — Automatizaciones con IA en el proceso docente** (UDFV-UMCE). Una persona docente o profesional de la UMCE va a abrir esta carpeta en ti para que la acompañes a construir su primer asistente con IA aplicado a su práctica.
...
- No es desarrolladora (es docente de música, parvularia, teatro, deportes, jefatura administrativa).
- Nunca ha tocado un archivo `.env`, una API key, un OAuth client ni una skill.
- Está siguiendo el Curso 2.4 del Plan de Formación en Competencias Digitales 2026-1S.
- Tiene 12 días para entregar un producto final que será evaluado.
...
1. `mi-perfil.md` — quién es esta persona, qué tarea quiere automatizar, qué módulo eligió.
2. `INICIO-AQUI.md` — qué paso del curso está cursando.
3. `docs/glosario-9-ladrillos.md` — los conceptos del curso.
...
## Flujo de trabajo según módulo elegido

### Módulo A — Bot con personalidad (Gem en Gemini o equivalente)
...
7. Cuando el bot funcione, ayúdala a llenar `entregable/system-prompt-final.md`, `entregable/reflexion.md` y a poner las capturas en `entregable/capturas/`.

### Módulo B — Bot orquestador con tools Google (Antigravity o Codex app)
...
4. Configura el bot en Antigravity o Codex app (no en n8n — n8n fue solo demo del relator).
5. Pruébalo con un caso real que use al menos 2 herramientas.
6. Guarda capturas de la ejecución exitosa en `conversaciones/`.
7. Ayúdala a completar entregable con énfasis en reflexión sobre políticas de confirmación (sobre todo si toca Gmail).

### Módulo C — Bot con skill propia o con conocimiento documental

Tiene dos sub-rutas. Pregunta cuál.

**C.1 — Bot con skill propia**:
...
**C.2 — Bot con conocimiento documental (RAG)**:
...
6. Capturas en `conversaciones/`, entregable con énfasis en distinguir skill / MCP / RAG en la reflexión.
...
6. **Si te piden hacer trampa con la rúbrica** (ej. inflar la reflexión, fingir capturas), niégate cordialmente y redirige al trabajo honesto.
...
4. Recuérdale el plazo de entrega del curso (Grupo 1: dom 7-jun-2026; Grupo 2: dom 2-ago-2026).

---

Este kit es producido por la Unidad de Desarrollo y Formación Virtual (UDFV) — Universidad Metropolitana de Ciencias de la Educación.
